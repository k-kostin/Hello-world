...
def main():
    ...
    if not price_file:
        ...
        # Предлагаем автоматически запустить парсинг
        print("\n" + "="*60)
        print("🚀 АВТОМАТИЧЕСКИЙ ЗАПУСК ПАРСИНГА")
        print("="*60)
        print("Хотите запустить парсинг всех регионов СЕЙЧАС?")
        print("⏱️  Время выполнения: ~10-15 минут")
        print("📊 Будет собрана информация по всем ~85 регионам России")
        print("🗺️  После парсинга автоматически создастся карта")
        
        while True:
            choice = input("\n👉 Запустить парсинг? [y/да/yes] или [n/нет/no]: ").lower().strip()
            if choice in ['y', 'да', 'yes', 'д']:
                print(f"\n[START] Запускаю парсинг всех регионов...")
                print("⏳ Это может занять 10-15 минут, пожалуйста подождите...")
                
                try:
                    # Запускаем парсинг всех регионов
                    result = subprocess.run([sys.executable, "regional_parser.py", "--all-regions"], 
                                          capture_output=False, text=True)
                    
                    if result.returncode == 0:
                        print(f"\n[SUCCESS] ✅ Парсинг завершён успешно!")
                        print("🔍 Ищу созданный файл с полной выгрузкой...")
                        
                        # Повторно ищем файл с данными
                        price_file = find_price_file()
                        if price_file:
                            print(f"[FOUND] ✅ Найден файл: {price_file}")
                            print("🗺️  Продолжаю создание карты...")
                            break  # Выходим из цикла и продолжаем создание карты
                        else:
                            print("[ERROR] ❌ Файл создан, но не найден. Проверьте вывод парсера выше.")
                            return False
                    else:
                        print(f"[ERROR] ❌ Ошибка парсинга (код: {result.returncode})")
                        print("💡 Попробуйте запустить вручную: python regional_parser.py --all-regions")
                        return False
                        
                except Exception as e:
                    print(f"[ERROR] ❌ Ошибка при запуске парсинга: {e}")
                    print("💡 Попробуйте запустить вручную: python regional_parser.py --all-regions")
                    return False
                    
            elif choice in ['n', 'нет', 'no', 'н']:
                print("\n[CANCEL] ❌ Парсинг отменён пользователем")
                print("💡 Для создания карты запустите парсинг вручную:")
                print("   python regional_parser.py --all-regions")
                return False
            else:
                print("❗ Введите 'да' для запуска или 'нет' для отмены")
                continue
