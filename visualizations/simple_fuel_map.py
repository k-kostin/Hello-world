#!/usr/bin/env python3
"""
Упрощенная карта без функции сравнения - только просмотр цен по регионам.
Используется как запасной вариант без сложной логики сравнения.
"""
...
class SimpleFuelMapGenerator:
    """Генератор простой карты с ценами на топливо без сравнения."""
    ...
    def create_map(self, output_path: str = "simple_fuel_map.html"):
        """Создает простую карту без функции сравнения."""
        ...
        for feat in gcopy["features"]:
            ...
            if region_prices:
                # Создание подробного попапа БЕЗ кнопки сравнения
                ...
                props["popup_html"] = popup_html
                props["base_color"] = "#90EE90"  # Светло-зеленый и для регионов без данных
        ...
        <div style="position: fixed; top: 10px; left: 50px; z-index: 1000; 
            ...
            <p style="margin: 0; color: #7f8c8d; font-size: 14px;">
                Кликните на регион для просмотра цен • Упрощенная версия
            </p>
        </div>'''
        ...
        """
        folium.Element(search_html).add_to(m)
        
        # CSS для улучшения интерфейса
        style_css = """
        <style>
        /* Перемещение кнопок масштаба ниже поля поиска */
        ...
        """
        folium.Element(style_css).add_to(m)

        # Простой JavaScript только для поиска и ограничений карты
        map_var = m.get_name()
        js_code = f"""
        <script>
        setTimeout(function() {{
        ...
        """
        folium.Element(js_code).add_to(m)
        
        m.save(output_path)
        print(f"[OK] Простая карта без сравнения сохранена: {output_path}")
        return m
...
def main():
    ...
    print(f"[OK] Создаем простую карту без функции сравнения...")
    
    generator = SimpleFuelMapGenerator(geojson_path, prices_path)
    generator.load_data()
    
    Path("data/maps").mkdir(parents=True, exist_ok=True)
    generator.create_map("data/maps/simple_fuel_map.html")
    
    print("[SUCCESS] Простая карта создана: data/maps/simple_fuel_map.html")
    print(f"[BROWSER] Откройте: file://{Path('data/maps/simple_fuel_map.html').absolute()}")
    print("[FEATURES] Возможности простой карты:")
    print("  • Светло-зеленая заливка для всех регионов")
    print("  • Просмотр цен по клику на регион")
    print("  • Поиск регионов")
    print("  • Ограничения карты в пределах России")
    print("  • БЕЗ функции сравнения регионов")
